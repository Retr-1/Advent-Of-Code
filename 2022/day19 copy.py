...
def main(time,robots,minerals,blueprint):
    ...
    if robots[0] >= blueprint[3][0] and robots[2] >= blueprint[3][1]:
        remain = 32-time
        return robots[3]*remain + minerals[3] + remain*(remain+1)/2


    best = (32-time)*robots[3] + minerals[3]
    ...
    if (32-time)*blueprint[3][1] > robots[2]*(32-time)+minerals[2]:
...
for i in range(3):
    memo = {}
    print(i)
    total *= main(0,(1,0,0,0),(0,0,0,0),blueprints[i])
print(total)